...
*** Database storage
These control what data gets stored in the database
| variable                                  | description                                                                                      |
|-------------------------------------------+--------------------------------------------------------------------------------------------------|
| org-sql-use-tag-inheritance               | use tag inheritance when populating the database                                                 |
| org-sql-ignored-properties                | properties to ignore                                                                             |
| org-sql-ignored-tags                      | tags to ignore                                                                                   |
| org-sql-ignored-link-types                | link types to ignore (eg mu4e, file)                                                             |
...
| org-sql-store-logbook-planning-changes    | whether to include logbook planning changes (eg changed deadline from x to y)                    |
| org-sql-store-logbook-state-changes       | whether to include logbook state changes (eg todo change from TODO to WAIT)                      |
| org-sql-store-logbook-other               | whether to include generic logbook entries that are not covered by other customization variables |
| org-sql-store-clocks                      | whether to include clocks                                                                        |
